...
public class RoundEndGravityPlugin : BasePlugin, IPluginConfig<RoundEndGravityConfig>
{
    ...
    private float _normalGravity = 800f;

    public override void Load(bool hotReload)
    {
        _normalGravity = ConVar.Find("sv_gravity")?.GetPrimitiveValue<float>() ?? 800f;
        Log.Info($"Server gravity captured: {_normalGravity}");
    ...
    {
        Server.ExecuteCommand($"sv_gravity {_normalGravity}");
        Log.Info($"sv_gravity reset to {_normalGravity}.");
        return HookResult.Continue;
    }
